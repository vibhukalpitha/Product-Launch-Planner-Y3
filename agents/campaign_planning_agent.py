...
Plans marketing campaigns based on target au                timeline['phases'] = []
                ...
                denom_ads = max(1, len(ads_data))
                for i, item in enumerate(ads_data):
                    insights = item.get('insights', {}).get('data', [{}])[0]
                    phase_duration = int(campaign_duration_days / denom_ads)
...
class CampaignPlanningAgent:
    def get_platform_campaign_details(self, platform: str, campaign_duration_days: int = 30) -> Dict[str, Any]:
        ...
        try:
            if platform.lower() == "youtube":
                # Use RSS feed for trending topics with better error handling
                ...
                try:
                    ...
                    if resp.content and b'<' in resp.content:
                        from xml.etree import ElementTree
                        ...
                        videos = []  # Fallback for invalid XML
                        
                except (requests.RequestException, ValueError) as e:
                    print(f"YouTube RSS feed error: {e}")
                    videos = []  # Fallback for parsing errors
                except Exception as e:
                    print(f"XML parsing error for YouTube RSS: {e}")
                    videos = []  # Fallback for XML parsing errors
                ...
                # Use trending topics from available RSS feeds with error handling
                ...
                try:
                    ...
                    if resp.content and b'<' in resp.content:
                        ...
                        trends = []  # Fallback for invalid XML
                        
                except (requests.RequestException, ValueError) as e:
                    print(f"Twitter trends RSS feed error: {e}")
                    trends = []  # Fallback for parsing errors
                except Exception as e:
                    print(f"XML parsing error for Twitter trends: {e}")
                    trends = []  # Fallback for XML parsing errors
    ...
    def discover_top_platforms(self, age_groups: List[str], top_n: int = 3) -> List[str]:
        ...
        for platform, age_data in default_platforms.items():
            ...
            # Defensive: avoid division by zero if age_groups is empty
            denom = max(1, len(age_groups))
            platform_scores[platform] = score / denom
    ...
    def fetch_platform_ad_cost(self, platform: str) -> float:
        ...
        """
        try:
            # Use the unified API manager instead of direct import
            from utils.unified_api_manager import UnifiedAPIManager
            api_manager = UnifiedAPIManager()
            
            serp_key = api_manager.get_working_key('SERP_API')
            if not serp_key:
                print("⚠️ SerpApi key not available, using fallback ad cost")
                return 1000.0
            
            # Use requests instead of serpapi library
            import requests
            
            # Search for platform advertising costs
            search_query = f"average monthly ad cost {platform}"
            ...
            if response.status_code == 200:
                ...
                cost = 1000.0  # Default fallback
                for item in organic_results[:3]:  # Check first 3 results
                    snippet = item.get('snippet', '')
                    # Look for cost patterns like $1,000, $500, etc.
                    import re
                    amounts = re.findall(r'\$(\d{1,3},?\d{0,3})', snippet)
                    if amounts:
                        try:
                            found_cost = float(amounts[0].replace(',', ''))
                            # Only use reasonable ad costs (minimum $100, maximum $50,000)
                            if 100 <= found_cost <= 50000:
                                cost = found_cost
                                break
                        except ValueError:
                            continue
                ...
                print(f"⚠️ SerpApi request failed: {response.status_code}, using fallback")
                return 1000.0
                
        except Exception as e:
            print(f"⚠️ SerpApi error: {e}, using fallback ad cost")
            return 1000.0
    ...
    def calculate_campaign_costs(self, platform_analysis: Dict[str, Any], target_budget: float, campaign_duration_days: int = 30) -> Dict[str, Any]:
        ...
        budget_analysis = {
            ...
            'budget_utilization': (total_estimated_cost / max(target_budget, 1)) * 100 if target_budget > 0 else 0,
    ...
    def _calculate_roi_projection(self, estimated_engagement: int, platform_budget: float) -> Dict[str, float]:
        ...
        roi = ((estimated_revenue - platform_budget) / max(platform_budget, 1)) * 100
        
        return {
            'estimated_conversions': estimated_conversions,
            'estimated_revenue': estimated_revenue,
            'roi_percentage': roi,
            'break_even_conversions': platform_budget / max(avg_order_value, 1)
        }
    ...
    def generate_campaign_recommendations(self, platform_analysis: Dict[str, Any],
        ...
        try:
            ...
            # Get marketing insights with error handling
            try:
                marketing_trends_url = "https://feeds.feedburner.com/socialmediatoday"
                resp = requests.get(marketing_trends_url, timeout=10)
                resp.raise_for_status()
                ...
                if resp.content and b'<' in resp.content:
                    ...
                    for item in root.findall('.//item')[:3]:
                        title_elem = item.find('title')
                        if title_elem is not None and title_elem.text:
                            marketing_insights.append(title_elem.text)
                else:
                    marketing_insights = []
                    
            except (requests.RequestException, ValueError) as e:
                print(f"Marketing trends RSS feed error: {e}")
                marketing_insights = []
            except Exception as e:
                print(f"XML parsing error for marketing trends: {e}")
                marketing_insights = []
    ...
    def _get_platform_specific_recommendations(self, platform: str) -> List[str]:
        ...
        try:
            ...
            print(f"Platform RSS feed error for {platform}: {e}")
            return []
        except Exception as e:
            print(f"XML parsing error for {platform}: {e}")
            return []
