...
try:
    ...
    logging.warning("Intelligent competitor discovery not available, using basic mapping")
...
class CompetitorTrackingAgent:
    def analyze_feedback_and_advise(self, feedback_list: list) -> str:
        ...
        """
        # Accept multiple input shapes: list of feedback dicts, a single sentiment-output dict
        # (with key 'sample_feedback'), or a mapping of competitor->sentiment dict.
        normalized = self._normalize_feedback_input(feedback_list)

        if not normalized:
            return "No feedback available to analyze."

        try:
            # Ensure every feedback item has a comment and a sentiment label
            for fb in normalized:
                # Ensure comment exists
                if not fb.get('comment'):
                    fb['comment'] = fb.get('title') or fb.get('headline') or ''

                # Infer sentiment label if missing
                if 'sentiment' not in fb or not fb.get('sentiment'):
                    score = simple_sentiment_analysis(fb.get('comment', ''))
                    fb['sentiment'] = self._infer_sentiment_label(score)

            # Group feedback by sentiment
            positives = [fb['comment'] for fb in normalized if fb.get('sentiment') == 'positive']
            negatives = [fb['comment'] for fb in normalized if fb.get('sentiment') == 'negative']
            neutrals = [fb['comment'] for fb in normalized if fb.get('sentiment') == 'neutral']
            ...
            total_feedback = len(normalized)
            if total_feedback > 0:
                pos_ratio = len(positives) / total_feedback
                neg_ratio = len(negatives) / total_feedback
                
                # Engagement analysis
                high_engagement = [fb for fb in normalized if fb.get('engagement', 0) > 100]
                if high_engagement:
    ...
    def __init__(self, coordinator):
        ...
        if real_data_available:
            ...
            if api_manager_available:
                ...
                self.use_real_data = True  # Default to using real data if available
            print("✅ Real Data Connector loaded")
        else:
            self.real_data_connector = None
            self.use_real_data = False
            print("⚠️ Real Data Connector not available")
        ...
        # Enhanced fallback competitors with more comprehensive lists
        self.fallback_competitors = {
            'smartphones': ['Apple', 'Google', 'OnePlus', 'Xiaomi', 'Huawei', 'Oppo', 'Vivo', 'Realme', 'Nothing', 'Motorola'],
            'tablets': ['Apple', 'Microsoft', 'Lenovo', 'Amazon', 'Samsung', 'Huawei', 'Xiaomi'],
            'laptops': ['Apple', 'Dell', 'HP', 'Lenovo', 'ASUS', 'Acer', 'MSI', 'Razer', 'Microsoft'],
            'wearables': ['Apple', 'Fitbit', 'Garmin', 'Amazfit', 'Xiaomi', 'Huawei', 'Samsung', 'Polar'],
            'tv': ['LG', 'Sony', 'TCL', 'Hisense', 'Vizio', 'Panasonic', 'Sharp'],
            'appliances': ['LG', 'Whirlpool', 'GE', 'Bosch', 'Electrolux', 'Samsung', 'KitchenAid', 'Maytag']
        }
    ...
    def _normalize_feedback_input(self, feedback_input) -> List[Dict[str, Any]]:
        """Normalize multiple possible feedback input shapes into a flat list of feedback dicts.

        Supported shapes:
        - List[Dict]: returned as-is (after shallow copy)
        - Dict with 'sample_feedback' key: returns that list
        - Dict mapping competitor->sentiment-dict: concatenates each 'sample_feedback' if present
        ...
        if isinstance(feedback_input, list):
            return [dict(fb) for fb in feedback_input if isinstance(fb, dict)]

        # If a dict with sample_feedback
        if isinstance(feedback_input, dict):
            if 'sample_feedback' in feedback_input and isinstance(feedback_input['sample_feedback'], list):
                return [dict(fb) for fb in feedback_input['sample_feedback'] if isinstance(fb, dict)]

            # If mapping of competitor -> sentiment dict
            normalized = []
            for comp, val in feedback_input.items():
                if isinstance(val, dict) and 'sample_feedback' in val and isinstance(val['sample_feedback'], list):
                    for fb in val['sample_feedback']:
                        if isinstance(fb, dict):
                            # attach competitor name when missing
                            if 'competitor' not in fb:
                                fb['competitor'] = comp
                            normalized.append(dict(fb))
    ...
    def _infer_sentiment_label(self, score: float) -> str:
        """Convert numeric sentiment score (-1..1) into a label string."""
        try:
            s = float(score)
        except Exception:
            return 'neutral'
    ...
    def _get_real_competitor_sentiment(self, competitor: str, category: str) -> Dict[str, Any]:
        """Get real sentiment data for competitor using multiple APIs"""
        try:
            # First try News API through real_data_connector
            if self.real_data_connector:
            ...
            # Fallback: Use SerpApi for competitor sentiment analysis
            print(f"📊 Using SerpApi fallback for {competitor} sentiment analysis")
            return self._get_serpapi_competitor_sentiment(competitor, category)
            
        except Exception as e:
            logging.error(f"Error getting real sentiment for {competitor}: {e}")
            # Final fallback: Use SerpApi
            return self._get_serpapi_competitor_sentiment(competitor, category)
    
    def _get_serpapi_competitor_sentiment(self, competitor: str, category: str) -> Dict[str, Any]:
        """Get competitor sentiment using SerpApi as fallback"""
        try:
            from utils.unified_api_manager import UnifiedAPIManager
            api_manager = UnifiedAPIManager()
            
            serp_key = api_manager.get_working_key('SERP_API')
            if not serp_key:
                return self._get_fallback_sentiment(competitor)
            
            import requests
            
            # Search for competitor reviews and sentiment
            search_query = f"{competitor} {category} reviews feedback sentiment"
            url = f"https://serpapi.com/search?q={search_query}&api_key={serp_key.key}"
            ...
            if response.status_code == 200:
                ...
                for result in organic_results[:5]:  # Analyze first 5 results
                    title = result.get('title', '')
                    snippet = result.get('snippet', '')
                    text = f"{title} {snippet}".lower()
                    
                    # Simple sentiment analysis
                    positive_words = ['good', 'great', 'excellent', 'positive', 'love', 'amazing', 'best', 'recommend']
                    negative_words = ['bad', 'poor', 'terrible', 'negative', 'hate', 'worst', 'awful', 'disappointed']
                    ...
                    sample_feedback.append({
                        'comment': title[:100] + '...' if len(title) > 100 else title,
                        'sentiment': sentiment,
                ...
                if total_results > 0:
                    ...
                    return {
                        'sentiment_scores': {
                            'positive': round(sentiment_percentages.get('positive', 0) / 100, 3),
                            'negative': round(sentiment_percentages.get('negative', 0) / 100, 3),
                            'neutral': round(sentiment_percentages.get('neutral', 0) / 100, 3)
                        },
                        'overall_score': sentiment_score,
                        'total_mentions': total_results,
                        'sample_feedback': sample_feedback[:3],
                        'data_source': 'SerpApi Search Results',
                        'recommendations': [
                            f"Monitor {competitor} sentiment trends",
                            "Analyze competitor strengths and weaknesses",
                            "Use insights for competitive positioning"
                        ]
                    }
            ...
            print(f"⚠️ SerpApi sentiment analysis failed: {e}")
            return self._get_fallback_sentiment(competitor)
    
    def _get_fallback_sentiment(self, competitor: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'sample_feedback': [
                {
                    'comment': f"Analysis for {competitor} - API data unavailable",
                    'sentiment': 'neutral',
                    'platform': 'System',
                    'engagement': 0,
                    'date': datetime.now().isoformat(),
            ...
            'recommendations': [
                "Monitor competitor activities manually",
                "Set up automated sentiment tracking",
